...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Animated, Dimensions, BackHandler } from "react-native"
...
const ExpandableCard: React.FC<CardProps> = ({ title, isExpanded, onToggle, children }) => {
  ...
  }, [isExpanded, animatedHeight]) // Added animatedHeight to dependencies

  return (
    <View style={styles.card}>
      <TouchableOpacity onPress={onToggle} style={styles.cardHeader}>
        <Text style={styles.cardTitle}>{title}</Text>
        <Icon name={isExpanded ? "chevron-up" : "chevron-down"} size={24} color="#fff" />
      </TouchableOpacity>
      <Animated.View
        style={[
          styles.cardContent,
  ...
  const renderPathNode = (level: number, icon: string, onPress: () => void) => {
    ...
    return (
      <TouchableOpacity onPress={onPress} disabled={!isUnlocked}>
        ...
        >
          <Icon name={icon} size={42} color={isUnlocked ? "#58cc02" : "#666"} />
          {!isUnlocked && (
      ...
      <View style={styles.header}>
        <View style={styles.headerContent}>
          <View style={styles.headerTextContainer}>
            <Text style={styles.welcomeText}>Welcome, {userData?.name || "Learner"}!</Text>
            {isFirstLogin && userData?.quizResults && (
              <View style={styles.quizResultsContainer}>
                <Text style={styles.levelText}>Level: {userData.quizResults.finalLevel}</Text>
                <Text style={styles.accuracyText}>Accuracy: {userData.quizResults.details?.accuracy || "0%"}</Text>
                <Text style={styles.motivationalText}>{getMotivationalQuote(userData.quizResults.finalLevel)}</Text>
              </View>
            )}
          </View>
        </View>
      ...
      <ScrollView style={styles.scrollView}>
        <View style={styles.content}>
          ...
          <View style={styles.learningPath}>
            <TouchableOpacity style={styles.startButton} onPress={() => router.replace("/(tabs)/q1" as any)}>
            ...
            <Animated.View style={[styles.star, showUnlockAnimation && { transform: [{ scale: unlockScale }] }]}>
              <TouchableOpacity
                onPress={() => router.replace("/(tabs)/star2q1" as any)}
                disabled={!userData?.unlockedStars || userData.unlockedStars < 2}
              >
                <Icon name="star" size={40} color={(userData?.unlockedStars ?? 0) >= 2 ? "#FFD700" : "#666666"} />
              </TouchableOpacity>
            </Animated.View>
            <View style={styles.pathLine} />

            {renderPathNode(3, "lighthouse", () => router.replace(`/(tabs)/q3` as any))}
            <View style={styles.pathLine} />
          ...
          <View style={styles.cardsContainer}>
            ...
            >
              <Text style={styles.cardText}>Complete 10 more lessons to start competing</Text>
            </ExpandableCard>
...
const styles = StyleSheet.create({
  ...
  header: {
    backgroundColor: "rgba(240, 101, 122, 0.95)",
    padding: 16,

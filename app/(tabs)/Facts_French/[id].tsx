...
import { StyleSheet, Image, TouchableOpacity, Text, View, Animated } from 'react-native';
...
export default function FactDetailScreen() {
  ...
  const playSound = async () => {
    if (fact) {
      const audioFile = AudioFiles[fact.audio.replace('.mp3', '') as keyof typeof AudioFiles];
      if (audioFile) {
    ...
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
        ...
        <TouchableOpacity onPress={toggleLanguage}>
          ...
          >
            <Text style={styles.text}>{isInFrench ? fact.textFr : fact.text}</Text>
          </LinearGradient>
        </TouchableOpacity>
